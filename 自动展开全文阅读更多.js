...
(function () {
    ...
    const rules = [{
            ...
            remove: [".btnForAppOpenA", ".btnForAppOpenImg", ".add_artilce_handle", ".WakeUptop"],
            content: [".art-content"]
        },  {
            // 环球网移动端
    ...
    for (const rule of rules) {
        ...
        if ("regex" in rule && rule.regex.test(window.location.href)) {
            hit = true;
        } else if ("wildcard" in rule && matchRule(window.location.href, rule.wildcard)) {
            hit = true;
        }
        if (hit) {
            let style = "";
            if ("remove" in rule && rule.remove.length > 0) {
                style += rule.remove.join(',\n') + `
                {
